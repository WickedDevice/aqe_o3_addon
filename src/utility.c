...
uint32_t get_r1(uint8_t sensor_index){
    return sensor_index == 0 ? O3_SENSOR_R1 : 0xFFFFFFFF;
}

uint32_t get_r2(uint8_t sensor_index){
    return sensor_index == 0 ? O3_SENSOR_R2 : 0xFFFFFFFF;
}

uint32_t get_r3(uint8_t sensor_index){
    return sensor_index == 0 ? O3_SENSOR_R3 : 0xFFFFFFFF;
}

uint16_t get_r1r2r3_threshold(uint8_t sensor_index){
    return sensor_index == 0 ? O3_R1R2R3_THRESHOLD : 0xFFFFFFFF;
}

uint16_t get_r1r2_threshold(uint8_t sensor_index){
    return sensor_index == 0 ? O3_R1R2_THRESHOLD : 0xFFFFFFFF;
}

uint32_t get_sensor_vcc(uint8_t sensor_index){
    return sensor_index == 0 ? O3_VCC_TENTH_VOLTS : 0xFFFFFFFF;
}
